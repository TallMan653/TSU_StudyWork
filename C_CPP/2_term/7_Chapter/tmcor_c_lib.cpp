...
{
    Usel* p = new Usel; //Берем память по новый элемент
    p->key = k; //записываем значение ключа
    p->next = head; //пишем старый адрес, для связи
    head = p; // новый адрес
}
void FStack::Push(float k)
{
    FUsel* p = new FUsel;
    p->key = k;
    p->next = head;
...
//Взять значение элемента с вершины стека и удалить его
int Stack::Pop()
{
    int k = head->key; //сохраняем информацию из вершины в память, на которую будет указывать k
    Usel* p = head; //Запоминаем указатель на вершину стека
    head = head->next; // переход на новую вершину
...
//Взять значение с вершины стека без удаления
int Stack::Top()
{
    return head->key; //Возвращаем ключ
}
float FStack::Top()
{
    return head->key; 
...
void toRPN(char p[], int Tab[]) {
    ...
    Stack Znak, Prior;
    int i, j, t;
    //Формируем массим для переменных
    for (i = 0; i < 256; i++) Tab[i] = 0;
    puts("Введите формулу:");
    gets_s(s);
    for (i = j = 0; s[i]; i++)
    {
        //Если нашли переменную меняем значения 0 на 0++
        if (str_small(s[i]) || str_large(s[i]))
        {
            Tab[s[i]]++;
            p[j++] = s[i];
            p[j++] = ' ';
        }
        //Если же число, то записываем в стек, и проверяем до первого не числа
        else if (str_number(s[i]))
        {
            p[j++] = s[i];
            i++;
            while (str_number(s[i]))
            {
                p[j++] = s[i++];
            }
            p[j++] = ' ';
        ...
        //если получается не цифра, и стек операций пустой или попался (
        else if (Znak.Empty() || s[i] == '(')
        {
            Znak.Push(s[i]); //Заносим знак в стек операции
            Prior.Push(Prioritet(s[i])); //Заносим его приоритет в стек приоритетов
        }
        else if (s[i] == ')') // Если попался )
        {

            //Рассматриваем до скобки (
            ...
            //Если не нашли, то ошибка в записи
            if (Znak.Empty())
            {
        ...
        //приоритет операции в стеке меньше приоритета нынешнего s[i]
        else if (Prior.Top() < Prioritet(s[i]))
        {
            //Помещаем операцию в стек операций и приоритета
            Znak.Push(s[i]);
            Prior.Push(Prioritet(s[i]));
        }
        else 
        {
            //Пока не пуст стек приоритетов или пока приоритет операции в стеке больше или равне приоритету текующий операции, то извлекаем в Полиз
            ...
            //После этого заносим текущую операцию
            Znak.Push(s[i]);
            Prior.Push(Prioritet(s[i]));
        }
    }
    //Извлекаем из стека все оставшиеся операции в ПОЛИЗ
    while (!Znak.Empty())
    {
        //Также проверяем на ошибку при потери )
        if (Znak.Top() == '(')
        {
            cout << endl << "Ошибка: потеряна ')'";
            exit(-1);
...
void fromRPN(char p[], int Tab[]) {
    //Берем из Tab, все переменные, если они есть, то пользователь должен ввести о них данные
    for (int i = 0; i < 256; i++)
    {
        if (Tab[i])
        {
    ...
    //Работаем с float, правда можно использовать и dooble, но цель лабараторный, я думаю больше в формирование и чтения ПОЛИЗ
    float x, y, z, k;
    //Раз работаем с float, то для этого нужен стек знаков с float
    FStack Calc;
    //Читаем полиз
    for (int i = 0; p[i];)
    {   
        //читаем полиз, в полиз только числа и знаки операторов или пробелы (последнее читается как символ) 
        while (p[i] && (str_number(p[i]) || str_small(p[i]) || str_large(p[i]) || p[i] == ' '))
        {
            //Если число, то переводим из char в float
            if (str_number(p[i]))
            {
                k = p[i++] - '0';
                //Если число не однозначное,то перевод требуется другой, рассматривали этот метод в перовм семестре
                while (str_number(p[i]))
                {
                    k = k * 10 + (p[i++] - '0');
                }
                //Записываем в стек число
                Calc.Push(k);
            }
            //Если операции, то записываем в просто как ссылку на символ через число;
            else if (str_small(p[i]) || str_large(p[i])) Calc.Push(Tab[p[i++]]);
            //Если пробел, то скипаем
            else if (p[i] == ' ') i++;
        }
        //Если p[i] не пустой
        if (p[i])
        {
            //Проверяем на пустату при чтение первого и второго числа, если получается иначе, то ошибка при ПОЛИЗ, или ещё что-то
            if (Calc.Empty())
            {
            ...
            x = Calc.Pop();
            //По следующиму читаем знак операции и производим это:
            switch (p[i++])
            {
            case '+': z = x + y; break;
            case '-': z = x - y; break;
    ...
    //После этого берем результат вычисления
    z = Calc.Pop();
    //Проверяем на ошибки
    if (!Calc.Empty())
    {
